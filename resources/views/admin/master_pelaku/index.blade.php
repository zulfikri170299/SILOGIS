        ...
        <div x-show="createModalOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="createModalOpen" class="inline-block p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-slate-800 border border-white/10 shadow-2xl rounded-2xl w-full max-w-md">
                    <h3 class="text-lg font-black text-white uppercase tracking-wider mb-4">Tambah Pelaku Pengadaan</h3>
                    <form action="{{ route('admin.master-pelaku.store') }}" method="POST">
                        @csrf
                        <div class="mb-4">
                            <label class="block text-xs font-bold text-slate-400 mb-2">Nama Peran (Contoh: KPA, PPK, PP)</label>
                            <input type="text" name="nama_peran" required class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-brand-primary focus:ring-brand-primary transition-all">
                        </div>
        ...
        <div x-show="editModalOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="editModalOpen" class="inline-block p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-slate-800 border border-white/10 shadow-2xl rounded-2xl w-full max-w-md">
                    <h3 class="text-lg font-black text-white uppercase tracking-wider mb-4">Edit Pelaku Pengadaan</h3>
                    <form :action="`/admin/master-pelaku/${selectedData.id}`" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-4">
                            <label class="block text-xs font-bold text-slate-400 mb-2">Nama Peran</label>
                            <input type="text" name="nama_peran" x-model="selectedData.nama_peran" required class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-brand-primary focus:ring-brand-primary transition-all">
                        </div>
                        <div class="flex justify-end gap-3 mt-6">
        ...
        <table class="w-full text-left text-sm text-slate-300">
            <thead class="bg-slate-900/80 text-[10px] uppercase tracking-widest font-black text-slate-500 border-b border-white/5">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-5">Nama Peran</th>
            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr class="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-6 py-4 font-bold text-white">{{ $item->nama_peran }}</td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-3">
                            ...
                            <form action="{{ route('admin.master-pelaku.destroy', $item) }}" method="POST" class="inline" onsubmit="return confirmDelete(this, 'Hapus data ini secara permanen?');">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="3" class="px-6 py-12 text-center">
                        <p class="text-slate-400 font-bold">Data Master Pelaku Pengadaan masih kosong.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
