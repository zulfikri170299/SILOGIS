    ...
    <nav class="lg:hidden bottom-nav">
        ...
        <div class="bottom-nav-center-wrap" x-data="{ showContact: false }">
            ...
            <div x-show="showContact" 
                ...
                @php
                    $igLink = $profile?->instagram ?? 'https://instagram.com';
                    $igUsername = trim(parse_url($igLink, PHP_URL_PATH), '/');
                    $igDmLink = $igUsername ? 'https://ig.me/m/' . $igUsername : 'https://instagram.com';
                @endphp
                <a href="{{ $igDmLink }}" target="_blank" class="flex items-center gap-3 px-4 py-3 bg-pink-500/10 hover:bg-pink-500/20 text-pink-500 rounded-xl transition-all font-bold text-xs uppercase tracking-wider group">
                    <svg class="w-5 h-5 group-hover:scale-110 transition-transform" viewBox="0 0 24 24" fill="currentColor">
