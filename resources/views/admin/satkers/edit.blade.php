...
<div class="mb-8">
    ...
    <h2 class="text-2xl font-black text-white font-outfit uppercase tracking-tight">Edit satker</h2>
</div>

<div class="max-w-2xl bg-slate-900/50 backdrop-blur-2xl rounded-3xl border border-white/5 shadow-2xl p-8">
    <form action="{{ route('admin.satkers.update', $satker) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Nama satker</label>
            <input type="text" id="name" name="name" value="{{ old('name', $satker->name) }}" required
        ...
        <div>
            ...
            <input type="number" id="urutan" name="urutan" value="{{ old('urutan', $satker->urutan) }}" placeholder="Contoh: 1, 2, 3..."
                class="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-all">
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                class="w-full bg-slate-950/50 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary transition-all">{{ old('description', $satker->description) }}</textarea>
            @error('description')
                <p class="mt-2 text-xs text-red-400">{{ $message }}</p>
            @enderror
        </div>
