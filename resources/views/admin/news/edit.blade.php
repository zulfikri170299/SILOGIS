...
<div class="max-w-4xl mx-auto">
    <div class="bg-slate-900/50 backdrop-blur-2xl rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden">
        <div class="bg-slate-900 px-10 py-8 flex justify-between items-center border-b border-white/5">
            <h2 class="text-white font-black tracking-tight font-outfit uppercase italic">Re-Komposisi Narasi</h2>
        ...
        <form action="{{ route('admin.news.update', $news) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            
            <div class="space-y-3">
                <label for="title" class="text-[10px] font-black uppercase tracking-widest text-slate-500">Narrative Title / Headline</label>
                <input id="title" name="title" type="text" value="{{ old('title', $news->title) }}" required autofocus
            ...
            <div class="space-y-4">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-8">
                    <div id="preview-container" class="shrink-0 h-44 w-72 bg-slate-800 rounded-2xl overflow-hidden border border-white/10 shadow-2xl flex items-center justify-center p-2 relative group">
                        @if($news->thumbnail)
                            <img id="image-preview" src="{{ asset('storage/' . $news->thumbnail) }}" alt="Preview" class="h-full w-full object-cover rounded-xl" />
                        @else
                            <img id="image-preview" src="#" alt="Preview" class="h-full w-full object-cover rounded-xl hidden" />
                        @endif
                    </div>
                    ...
                    <div class="flex-1 w-full">
                        <label class="relative cursor-pointer bg-slate-800/50 hover:bg-brand-primary/20 text-slate-300 hover:text-white font-black py-10 rounded-2xl border-2 border-dashed border-white/10 transition-all flex flex-col items-center justify-center group overflow-hidden">
                            ...
                            <span class="text-[10px] uppercase tracking-widest text-center italic">Ganti Thumbnail (Optional)</span>
            ...
            <div class="space-y-3">
                ...
                <div class="relative group">
                    ...
                    <input id="instagram_url" name="instagram_url" type="url" value="{{ old('instagram_url', $news->instagram_url) }}"
            ...
            <div class="space-y-3">
                ...
                <textarea id="content" name="content" rows="12" required
                    class="block w-full bg-slate-800/50 rounded-[2rem] border-white/5 py-8 px-10 text-white shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-lg font-medium leading-relaxed transition-all placeholder:text-slate-600">{{ old('content', $news->content) }}</textarea>
                @error('content') <p class="text-[10px] text-rose-500 font-bold uppercase tracking-widest">{{ $message }}</p> @enderror
            </div>

            <div class="pt-10 border-t border-white/5 flex justify-end gap-6">
                <a href="{{ route('admin.news.index') }}" class="px-8 py-5 rounded-2xl font-black text-slate-500 hover:text-white transition-colors uppercase tracking-widest text-[10px] italic">Abort Sequence</a>
