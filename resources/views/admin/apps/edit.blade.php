...
<div class="max-w-3xl mx-auto">
    <div class="bg-slate-900/50 backdrop-blur-2xl rounded-[2.5rem] border border-white/5 shadow-2xl overflow-hidden">
        <div class="bg-slate-900 px-10 py-8 flex justify-between items-center border-b border-white/5">
            <h2 class="text-white font-black tracking-tight font-outfit uppercase italic">Re-Konfigurasi Aset</h2>
        ...
        <form action="{{ route('admin.apps.update', $app) }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="space-y-3">
                    ...
                    <input id="title" name="title" type="text" value="{{ old('title', $app->title) }}" required autofocus
            ...
            <div class="space-y-3">
                ...
                <input id="url" name="url" type="url" value="{{ old('url', $app->url) }}" required
                    class="block w-full bg-slate-800/50 rounded-2xl border-white/5 py-4 text-white shadow-sm focus:border-brand-primary focus:ring-brand-primary sm:text-sm font-bold transition-all placeholder:text-slate-600"
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-8">
                    <div id="icon-preview-container" class="shrink-0 h-24 w-24 bg-white rounded-2xl overflow-hidden border border-white/10 shadow-2xl flex items-center justify-center p-3">
                        @if($app->icon)
                            <img id="icon-preview" src="{{ asset('storage/' . $app->icon) }}" alt="Preview" class="h-full w-full object-contain" />
                        @else
                            <img id="icon-preview" src="#" alt="Preview" class="h-full w-full object-contain hidden" />
                        @endif
                    </div>
                    <div class="flex-1">
                        <label class="cursor-pointer bg-slate-800/50 hover:bg-brand-primary/20 text-slate-300 hover:text-white font-black py-4 px-8 rounded-2xl border-2 border-dashed border-white/10 transition-all inline-block">
                            <span class="text-[10px] uppercase tracking-widest">Update Master Icon</span>
                            <input type="file" id="icon" name="icon" accept="image/*" onchange="previewIcon(event)" class="sr-only"/>
                        </label>
                        <p class="text-[9px] text-slate-500 mt-3 uppercase font-black tracking-widest italic">Biarkan kosong jika tetap menggunakan versi saat ini.</p>
                    </div>
                </div>
            ...
            <div class="space-y-3">
                ...
                <textarea id="description" name="description" rows="4"
                    ...
                    placeholder="Jelaskan kegunaan aplikasi ini dalam 1-2 kalimat...">{{ old('description', $app->description) }}</textarea>
            ...
            <div class="pt-10 border-t border-white/5 flex justify-end gap-6">
                <a href="{{ route('admin.apps.index') }}" class="px-8 py-5 rounded-2xl font-black text-slate-500 hover:text-white transition-colors uppercase tracking-widest text-[10px] italic">Abort Sequence</a>
